...
//		$('body').wrapInner('<div id="superContainer" />');
		//$('body #canvas .wp-pagescroll_content').wrapInner('<div id="superContainer" />');
		$('body #canvas').wrapInner('<div id="superContainer" class="superContainerFlag" />');
                //superContainerFlag 鍒犻櫎鏍囪
		//creating the navigation dots 
		if (options.navigation) {
			//$('body #canvas .wp-pagescroll_content').append('<div id="fullPage-nav"><ul></ul></div>');
		...
		$('.section').each(function(index){
			...
			//$(this).find('.bg > img').attr('src','http://sitestar2.wopop.com/plugin/pagescroll/images/section'+j+'.jpg');
			if(!index){
...
//		$('.section').on('click', '.toSlide', function(e) {
		...
		*/
		...
		if (!isTablet) {
			...
			$(window).resize(function() {
				...
				if(obj && $(obj).parent().is('.img_over,.ui-draggable,ui-resizable')) return false;
		...
		/**
		 * When resizing is finished, we adjust the slides sizes and positions 鎷栧姩绐楀彛锛岃嚜閫傚簲绐楀彛澶у皬缂╂斁 榛樿澶勭悊
		 */
		...
		*/
		...
		/**
		* Scrolls the slider to the given slide destination for the given section 婊氬姩婊戝潡鏉ョ粰骞荤伅鐗囩洰鐨勫湴瀹氭
		*/
...
//                        setTimeout(function(){
        $('.section').each(function(i){
            ...
            if($.fullpagecanvasleft()>0){
                ...
                $(this).find('.full_pagescroll').css({"margin-left":$.fullpagecanvasleft() + "px"});//娴忚鍣ㄥぇ灏忕缉鏀惧鐞�
                //鎵嬫満妯″潡澶勭悊 str
                if($.wismobile){
                    if($.wiseditmode){ //缂栬緫涓�
                            $(this).css({"width":($('#canvas').width()-0) + "px","left":"0px"});
                          //  $(this).find('.fullimgc').css({"width":($('#canvas').width()-0) + "px","left":"0px"});
                        ...
                        if($.wp_fullpag_mobile()==false){
                            $(this).css({"width":($('#canvas').width()-0) + "px","left":"0px"});
                          //  $(this).find('.fullimgc').css({"width":($('#canvas').width()-0) + "px","left":"0px"});
                            $(this).find('.full_pagescroll').css({"width":$('#canvas').width() + "px","height":$(this).height() + "px","margin-left":"0px"});
                ...
                $(this).css({"left":"0px"});//娴忚鍣ㄥぇ灏忕缉鏀惧鐞�
                $(this).find('.full_pagescroll').css({"margin-left":"0px"});//娴忚鍣ㄥぇ灏忕缉鏀惧鐞�
            }
            var $paragraph_image = $(this).find('.section .bg > img');//瀛樺湪娌℃壘鍒�
            // 鑾峰彇鏈€澶у尯鍩熻寖鍥�
